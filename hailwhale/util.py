...
# convert the json argument in python object 
def try_loads(arg):
...
# convert the python object and convert them in json object
...
# return parent dimension for passes subdimension
def parent(sub):
    sub = try_loads(sub)
    if sub == '_':
        return None
...
class whale_cache(object):
    ...
    def __call__(self, *args, **kwargs):
        ...
        if 'period' in kwargs:
            ...
            kwargs['period'] = str(p)
            ttl = int (parseUnit(p.interval) / 5)
        else:
            ttl = 60
